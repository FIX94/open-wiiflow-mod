...
void wii_menu_show()
{
  ...
  wii_menu_quit_loop = 0;
	...
	if(main_argc > 3 && main_argv[1] != NULL && main_argv[2] != NULL && main_argv[3] != NULL)
	{
		test = 3;
      ...
      else
      {
        if( delay_frames < 0 || test)
        {
            ...
            wii_analog_right( expX, gcX ) )
          {
            ...
            wii_analog_down( expY, gcY ) ) || test == 2)
          {
            wii_menu_move( menu, 1 );
			if(test)
			{
				TREENODE* node = menu->children[menu_cur_idx];
				if(strcasecmp(node->name, main_argv[2]) == 0)
					test--;
			}
			else
				wii_menu_force_redraw = 1;
