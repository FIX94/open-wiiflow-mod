...
bool NandSave::CheckSave()
{
	...
	free(certBuffer);
	free(u8_bin);
	printf("Created WiiFlow Save\n");
done:
	loaded = true;
	return loaded;

error:
	printf("Error while creating WiiFlow Save\n");
	loaded = false;
	ES_AddTitleCancel();
	if(certBuffer != NULL)
		free(certBuffer);
...
void NandSave::LoadSettings()
{
	...
	{
		printf("Loading IOS Settings from NAND\n");
		cur_ios = file->cios;
	...
	{
		printf("Using Port Settings from NAND\n");
		cur_port = port[0] & 1;
	}
	if(port != NULL)
		free(port);
...
void NandSave::SaveIOS()
{
	...
	printf("Saving IOS Settings to NAND\n");
	WriteFile(IOS_SAVE_PATH, (u8*)&ios_settings, sizeof(ios_settings_t));
}

void NandSave::SavePort()
{
	if(loaded == false)
		return;
	printf("Saving Port Settings to NAND\n");
	WriteFile(PORT_SAVE_PATH, &cur_port, sizeof(cur_port));
}
