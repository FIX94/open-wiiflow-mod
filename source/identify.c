...
int PatchIOS(bool patch_all)
{
	...
	write16(MEM_PROT, 0);
	/* Do Patching */
	patches += apply_patch("isfs_permissions", isfs_perm_old, isfs_perm_patch, sizeof(isfs_perm_patch));
	if(patch_all)
	{
		patches += apply_patch("es_setuid", setuid_old, setuid_patch, sizeof(setuid_patch));
		patches += apply_patch("es_identify", es_identify_old, es_identify_patch, sizeof(es_identify_patch));
		patches += apply_patch("hash_check", hash_old, hash_patch, sizeof(hash_patch));
		patches += apply_patch("new_hash_check", new_hash_old, new_hash_patch, sizeof(new_hash_patch));
	}
	/* Reinit */
	write16(MEM_PROT, 1);
	__ES_Init();
